-- 09_set operators
-- set 연산자는 두 개 이상의 select 문의 결과 집합을 결합하는데 사용된다.
...
-- 두 select 문의 결과 중 공통되는 레코드만 변환한다.
SELECT
    menu_code,
    menu_name,
    menu_price,
...
-- 4. EXCEPT
-- 첫 번째 select문의 결과에서 두번째 select 문의 결과에 포함된 레코드를 제외한 결과 반환
-- 일부 dbms에서는 minus 라는 이름을 사용
SELECT
    menu_code,
    menu_name,
    menu_price,
