...
-- 일반적으로 두 테이블은 공통된 의미를 가지는 컬럼(키)를 기준으로 결합하며 이를 join 조건이라고 한다.
...
-- inner join은 조건에 일치하는 행만 반환하므로 employee에서 dept_code가 null인 사원은 포함되지 않는다.
-- 일치하지 않는 행도 포함하여 조회하려면 outer join을 사용해야 한다.

-- using을 이용한 inner join
-- 두 테이블에서 join 기준 칼럼명이 동일한 경우에 사용할 수 있다.
-- 결과 집합에서도 해당 컬럼이 하나만 표시된다.
SELECT
    a.menu_name,
    b.category_code
FROM
...
-- (1) self join
-- 같은 테이블을 두 번 이상 참조하여 동일 테이블 내의 행과 행 사이의 관게를 조회하는 join 방식
