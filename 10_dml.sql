-- 10_dml(데이터 조작 언어)
-- 테이블에 데이터를 삽입(insert), 수정(uqdate), 삭제(delete)할 때 사용하는 sql 구문
...
INSERT INTO
    tbl_menu
VALUES  -- menu_code 컬럼은 AUTO_INCREMENT 설정이 되어있으므로 NULL 처리
...
-- where 절에서 삭제 대상 행을 반드시 선택해야 하며 생략할 경우 테이블의 전체 행이 삭제된다.
DELETE FROM
    tbl_menu
WHERE
    menu_code = 24;
